...
const SettingsView: React.FC<SettingsViewProps> = (props) => {
    ...
    const [sbKey, setSbKey] = useState(window.localStorage.getItem('SUPABASE_ANON_KEY_OVERRIDE') || '');
    ...
    const handleSaveDatabase = () => {
        if (sbUrl) window.localStorage.setItem('SUPABASE_URL_OVERRIDE', sbUrl.trim());
        else window.localStorage.removeItem('SUPABASE_URL_OVERRIDE');
        
        if (sbKey) window.localStorage.setItem('SUPABASE_ANON_KEY_OVERRIDE', sbKey.trim());
        else window.localStorage.removeItem('SUPABASE_ANON_KEY_OVERRIDE');
        
        alert('Datenbank-Einstellungen gespeichert. Die Seite wird nun neu geladen, um die Verbindung herzustellen.');
        window.location.reload();
    };

    const renderDatenbankTab = () => (
        <div className="settings-section">
            <div className="settings-section-header">
                <h3 className="settings-section-title">Supabase Verbindung</h3>
            </div>
            <div className="settings-section-body">
                <p className="form-group-description">
                    Geben Sie hier Ihre Supabase-Zugangsdaten ein, um die Echtzeit-Synchronisation zwischen verschiedenen Browsern zu aktivieren.
                </p>
                <div className="form-group">
                ...
                <div className="form-group">
                    ...
                    <input 
                        ...
                        placeholder="Ihr langer anon public key" 
                        className="form-group-input" 
                    />
                </div>
                <button onClick={handleSaveDatabase} className="btn btn-primary" style={{ marginTop: '1rem' }}>
                    Verbindung speichern & App neu starten
                </button>
                
                <div style={{ marginTop: '2rem', padding: '1rem', background: 'rgba(13, 110, 253, 0.05)', borderRadius: '8px', border: '1px solid rgba(13, 110, 253, 0.1)' }}>
                    <h4 style={{ margin: '0 0 0.5rem 0', fontSize: '0.9rem' }}>Anleitung:</h4>
                    <ol style={{ margin: 0, paddingLeft: '1.2rem', fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                        <li>Erstellen Sie ein Projekt auf <strong>supabase.com</strong>.</li>
                        <li>Führen Sie den SQL-Befehl aus (siehe Chat) um die Tabelle <code>app_data</code> zu erstellen.</li>
                        <li>Kopieren Sie die URL und den Anon Key aus den <strong>Project Settings {"->"} API</strong>.</li>
                        <li>Speichern Sie hier und laden Sie die Seite neu.</li>
                    </ol>
                </div>
            </div>
        </div>
